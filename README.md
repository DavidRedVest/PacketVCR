...
> **macOS 提示"PacketVCR 已损坏，无法打开，你应该将它移到废纸篓"**：CI 里已经加了修复
> （`macdeployqt` 打包 Qt 运行时后会破坏可执行文件原有的签名，导致 Gatekeeper 判定"已损坏"这种
> 无法绕过的硬拦截；CI 现在会在打包后重新做 ad-hoc 签名修复这个问题），新发布的包不应该再遇到
> 这条报错。如果你手上的包是旧版本，或者依然遇到这个提示，可以在终端执行：
> ```sh
> xattr -cr /path/到/PacketVCR.app
> ```
> 这个命令只能清掉"下载隔离"标记（未通过 Apple 公证导致的"未知开发者"提示），如果签名本身是
> 坏的（老版本的包），这条命令不会有用，去下载最新的 Release 即可。
